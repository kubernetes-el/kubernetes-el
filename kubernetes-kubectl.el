...
(defun kubernetes-kubectl-get-deploymentconfigs (props state cb &optional cleanup-cb)
  "Get all deploymentconfigs and execute callback CB with the parsed JSON.
...
CLEANUP-CB is a function taking no arguments used to release any resources."
  (kubernetes-kubectl props state '("get" "deploymentconfigs" "-o" "json")
                      (lambda (buf)
                        (let ((json (with-current-buffer buf
                                      (json-read-from-string (buffer-string)))))
                          (funcall cb json)))
...
CLEANUP-CB is a function taking no arguments used to release any resources."
  (kubernetes-kubectl props state `("get" ,kubernetes-namespaces-resource "-o" "json")
                      (lambda (buf)
                        (let ((json (with-current-buffer buf
                                      (json-read-from-string (buffer-string)))))
                          (funcall cb json)))
...
(defun kubernetes-kubectl-delete-deploymentconfig (props state deploymentconfig-name cb &optional error-cb)
  "Delete DEPLOYMENTCONFIG-NAME, then execute CB with the response buffer.
...
ERROR-CB is called if an error occurred."
  (kubernetes-kubectl props state (list "delete" "deploymentconfig" deploymentconfig-name "-o" "name")
                      (lambda (buf)
                        (with-current-buffer buf
                          (string-match (rx bol "deploymentconfig/" (group (+ nonl))) (buffer-string))
                          (funcall cb (match-string 1 (buffer-string)))))
                      error-cb))
