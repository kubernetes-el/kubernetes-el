...
(ert-deftest kubernetes-logs-test--read-resource-with-prompt-fallback ()
  "Test kubernetes-logs--read-resource-with-prompt fallback implementation."
  (let ((kubernetes-logs-supported-resource-types '("pod" "deployment"))
        (kubernetes-logs--selected-resource nil)
        (resource-type-selected nil)
        (resource-name-selected nil))

    ;; Test with kubernetes-describe--get-resource-name not available
    (cl-letf (((symbol-function 'fboundp)
               (lambda (sym) (not (eq sym 'kubernetes-describe--get-resource-name)))) ;; Make describe function unavailable
              ((symbol-function 'completing-read)
               (lambda (prompt collection &rest _)
                 (cond
                  ((string-match-p "Resource type:" prompt)
                   (setq resource-type-selected "pod")
                   "pod")
                  ((string-match-p "pod name:" prompt)
                   (setq resource-name-selected "pod1")
                   "pod1")
                  (t nil))))
      ...
      ;; Call the function with state parameter
      (let ((result (kubernetes-logs--read-resource-with-prompt '((current-namespace . "default"))))))

      ;; Verify selections were made
      (should (equal resource-type-selected "pod"))
      (should (equal resource-name-selected "pod1")))))

(ert-deftest kubernetes-logs-test--read-resource-with-prompt-empty-results ()
  "Test kubernetes-logs--read-resource-with-prompt with empty results."
  (let ((kubernetes-logs-supported-resource-types '("pod" "deployment"))
        (kubernetes-logs--selected-resource nil)
        (resource-type-selected nil)
        (deployment-name-entered nil))

    ;; Test with kubernetes-describe--get-resource-name not available and empty results
    (cl-letf (((symbol-function 'fboundp)
               (lambda (sym) (not (eq sym 'kubernetes-describe--get-resource-name)))) ;; Make describe function unavailable
              ((symbol-function 'completing-read)
               (lambda (prompt collection &rest _)
                 (when (string-match-p "Resource type:" prompt)
                   (setq resource-type-selected "deployment")
                   "deployment")))
              ((symbol-function 'kubernetes-kubectl-await-on-async)
               (lambda (state fn)
                 '((items . ())))) ;; Empty items
              ((symbol-function 'read-string)
               (lambda (prompt &rest _)
                 (when (string-match-p "deployment name:" prompt)
                   (setq deployment-name-entered "my-deployment")
                   "my-deployment"))))
      ...
      ;; Verify selections were made
      (should (equal resource-type-selected "deployment"))
      (should (equal deployment-name-entered "my-deployment")))))
