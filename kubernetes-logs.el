...
Returns a cons cell of (type . name)."
  (condition-case nil
      (let* ((resource-type (completing-read "Resource type: " kubernetes-logs-supported-resource-types nil t))
             (resource-name (if (fboundp 'kubernetes-describe--get-resource-name)
                               (kubernetes-describe--get-resource-name state resource-type)
                             ;; Fallback implementation if kubernetes-describe is not loaded
                             (let* ((resource-plural
                                     (if (string-suffix-p "s" resource-type)
                                         resource-type
                                       (concat resource-type "s")))
                                    (resources (kubernetes-kubectl-await-on-async state
